            ...
            <div class="col-xxl-2 col-lg-3 col-sm-4 col-6">
                <div class="payment-box text-center">
                    ...
                    <div class="payment-box-content">
                        <h4 class="title">
                            @if (strtolower($gateway->name) == 'global-pay' || strtolower($gateway->name) == 'interac')
                            Debit / Credit Card
                        @else
                            {{ ucwords(str_replace('_', ' ', $gateway->name)) }}
                        @endif
                        </h4>
                        <button data-href="{{ route('user.paynow', $gateway->id) }}" data-id="{{ $gateway->id }}"
                            class="btn sp_theme_btn btn-sm w-100 paynow mt-3">Deposit Now</button>
                    </div>
                </div>
            </div>
        @empty
                ...
                <form action="" method="post">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-sm sp_btn_secondary"
                                data-bs-dismiss="modal">{{ __('Close') }}</button>
                            <button type="submit" class="btn btn-sm sp_theme_btn">Deposit Now</button>
                        </div>
                    </div>
                </form>
            </div>
